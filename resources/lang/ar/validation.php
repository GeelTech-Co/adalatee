<?php

return [
    'required' => 'حقل :attribute مطلوب.',
    'string' => 'يجب أن يكون :attribute نصًا.',
    'max' => [
        'string' => 'يجب ألا يزيد :attribute عن :max حرفًا.',
        'file' => 'يجب ألا يزيد :attribute عن :max كيلوبايت.',
    ],
    'email' => 'يجب أن يكون :attribute بريدًا إلكترونيًا صالحًا.',
    'unique' => 'الـ :attribute مستخدم بالفعل.',
    'confirmed' => 'تأكيد :attribute غير متطابق.',
    'regex' => 'تنسيق :attribute غير صالح.',
    'exists' => 'الـ :attribute المختار غير صالح.',
    'in' => 'الـ :attribute المختار غير صالح.',
    'image' => 'يجب أن يكون :attribute صورة.',
    'mimes' => 'يجب أن يكون :attribute ملف من نوع: :values.',

    'attributes' => [
        'name' => 'الاسم',
        'email' => 'البريد الإلكتروني',
        'password' => 'كلمة المرور',
        'old_password' => 'كلمة المرور القديمة',
        'password_confirmation' => 'تأكيد كلمة المرور',
        'type' => 'نوع التمرين',
        'muscle_group' => 'مجموعة العضلات',
        'secondary_muscles' => 'العضلات الثانوية',
        'description' => 'الوصف',
        'instructions' => 'التعليمات',
        'precautions' => 'الاحتياطات',
        'image' => 'الصورة',
        'animation' => 'الرسوم المتحركة',
        'token' => 'الرمز',
        'id' => 'المعرّف',
        'hash' => 'التجزئة',
        'plan_id' => 'معرّف الخطة',
        'day_id' => 'معرّف اليوم',
        'custom_plan_id' => 'معرّف الخطة المخصصة',
        'custom_day_id' => 'معرّف اليوم المخصص',
        'start_time' => 'وقت البداية',
        'end_time' => 'وقت النهاية',
        'status' => 'الحالة',
        'description_en' => 'الوصف (بالإنجليزية)',
        'description_ar' => 'الوصف (بالعربية)',
        'exercise_logs' => 'سجلات التمارين',
        'exercise_logs.*.exercise_id' => 'معرّف التمرين',
        'exercise_logs.*.round_number' => 'رقم الجولة',
        'exercise_logs.*.reps' => 'التكرارات',
        'exercise_logs.*.weight' => 'الوزن',
    ],
];